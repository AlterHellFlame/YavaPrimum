...
export class CandidatePageComponent implements OnInit {
    ...
    return this.candidates.filter(
      (candidate) =>
        candidate.candidate.surname
          .toLowerCase()
          .includes(this.filter.surname.toLowerCase()) &&
  ...
  getTaskEntries(tasks: Tasks[]): { key: string; value: string }[] {
    return tasks.map((task) => ({
      key: DateTime.fromISO(task.dateTime.toString()).toFormat(
        'dd.MM.yyyy HH:mm'
      ),
      value: task.status,
    }));
  ...
  canChangeDateTime(tasks: Tasks[] = this.selectedTasks || []): string | null {
    ...
    if (tasks.some((task) => ['Пришел', 'Не пришел'].includes(task.status))) {
      return null;
    }

    const statusesToCheck = ['Время подтверждено', 'Назначен приём'];

    if (
      tasks.some(
        (task) => statusesToCheck.includes(task.status) && !task.isArchive
      )
    ) {
      return 'datetime';
    }
  ...
  toggleCandidateDetails(candidate: CandidatesFullData): void {
    this.expandedCandidates[candidate.candidate.candidateId!] =
      !this.expandedCandidates[candidate.candidate.candidateId!];
  }
}
